...
class RaspberryChargerSensor(Entity):
    ...
    def update(self):
        ...
        if _throttled == '0':
            self._description = 'Normal'
        elif _throttled == '1000':
            self._description = 'Under-voltage (bad psu)'
        elif _throttled == '2000':
            self._description = 'Limited (bad psu)'
        elif _throttled == '3000':
            self._description = 'Limited (bad psu)'
        elif _throttled == '4000':
            self._description = 'Throttled (bad psu)'
        elif _throttled == '5000':
            self._description = 'Throttled (bad psu)'
        elif _throttled == '8000':
            self._description = 'Overheating.'
        else:
            self._description = 'Issue with power supply or system.'
        if self._text_state:
